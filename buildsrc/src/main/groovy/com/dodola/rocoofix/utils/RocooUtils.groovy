    ...
    static Set<File> getDexTaskInputFiles(Project project, BaseVariant variant, Task dexTask) {
        ...
        if (isUseTransformAPI(project)) {
            ...
            dexTask.inputs.files.files.each {
                println("paths" + it.absolutePath)
                if (it.exists()) {
                    println("exist paths" + it.absolutePath)
                    if (it.isDirectory()) {
                        Collection<File> jars = FileUtils.listFiles(it, extensions, true);
                        files.addAll(jars)

                        String s = "intermediates\\classes\\" + transformSlash("${variant.dirName}");
                        if (it.absolutePath.toLowerCase().endsWith(s.toLowerCase())) {
                            files.add(it)
                        }
...
//        println("==========gradleVersion:" + compareVersionName("1.9.0", "1.4.0"))
    ...
    public static String transformSlash(String s) {
        return s.replace('/', '\\');
    }

    public static String subPath(String path, String split) {
        String path1 = path.replace("\\", '/');
        String split1 = split.replace("\\", '/');
        String sub = path1.split(split1)[1];
        return sub.replace('/', '\\');
    }
